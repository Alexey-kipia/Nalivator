...
void oled_Servo_calibr(
    uint8_t subServo);  // Экран меню НАСТРОЙКИ СЕРВО КАЛИБРОВКА
...
void oled_Kalibr_Pump(
    uint8_t subPump);  // Экраны меню "НАСТРОЙКИ" -> "КАЛИБРОВКА ПОМПЫ"
void oled_Num_Folder(
    uint8_t subFolder);  // Настройки -> Тосты -> Папка с треками
...
void oled_Bar_Man(
    uint8_t subBarmen);  // Экран меню Настройки -> Бармен-Долив
...
#if (DEBUG_UART == 0)
                         //#define Button_Tower        // сенсорная кнопка на
                         //башне. Раскомментировать если есть
#endif
//#define BAT_MONITOR_ON          // включение в коде всё что звязано с АКБ,
//закомментировать если нет АКБ вообще.
#ifdef BAT_MONITOR_ON
#define ADC_U_COEFF \
    0.00472  // коэффициент перевода кода АЦП в напряжение.  1.1 / 1023 / R4 *
             // (R4 + R5).  Условие должно выполняться - R4 / ( R4 + R5 ) = 0.22
#define LOW_BAT_SLEEP_ON  // контроль низкого заряда акб. Закомментировать -
                          // выключить для отладки кода, если не подключен пин
                          // A3 к мониторингу АКБ, иначе сон
#endif
#define SOUND_THEME 0  // звуковая тема, 0 - по умолчанию, 1 - для танкистов
#define NUM_SHOTS \
    5  // количество рюмок (оно же кол-во светодиодов и концевиков!), всё
       // делалось под 6 рюмок!!!!
#define SWITCH_LEVEL \
    1  // концевики 1 - высокий сигнал при замыкании, 0 - низкий
#define PUMP_LEVEL \
    1  // уровень включения помпы, 1 - высокий уровень, 0 - низкий
#define MAX_DRINK \
    50  // максимальный объём тары мл, для корректного отображения в менюшках
        // экрана, ставить не более 95
...
#define MEMORY_ON  // включение записи параметров в память, закомментировано -
                   // значит выключено!
#define START_POS_SERVO_GLASS1  // раскомментировать, если нужна начальная
                                // позиция серво в первой рюмке
//#define SERVO_CHANGE_DIRECTION    // раскомментировать, отзеркалить движение
//серво
#ifdef SERVO_CHANGE_DIRECTION
#define INITAL_ANGLE_SERVO \
    180  // начальный угол на который становится серво, если включен режим
         // зеркало, подбирать в меньшую сторону, если упирается серво.
#endif

// изменить значения если серво не доезжает до нужных углов, не всегда
// эффективно!
#define SERVO_MIN 544  // уменьшить если не доезжает до 0° (544 по умолчанию)
#define SERVO_MAX \
    2400  // увеличить если не доезжает до 180° (2400 по умолчанию )
...
const uint8_t SW_pins[] = {A0, A1, A2,
                           6,  7,  8};  // Пины концевиков для Arduino NANO
#else
const uint8_t SW_pins[] = {8,  7,  A3,
                           A2, A1, A0};  // Пины концевиков для Arduino NANO
#endif
...
#define SERVO_PIN \
    2  // Пин серво для Arduino mega. Можно только 2, 3, 5, 6, 7, 8, 11, 12, 13,
       // 44, 45, 46 Пины!!!!!!!!
#ifndef SERVO_CHANGE_DIRECTION
const uint8_t SW_pins[] = {A0, A1, A2,
                           A3, A7, A8};  // Пины концевиков для Arduino mega
#else
const uint8_t SW_pins[] = {A8, A7, A3,
                           A2, A1, A0};  // Пины концевиков для Arduino mega
#endif
...
#endif
#define BUSY_PIN \
    10  // Пин готовности DF плеера для Arduino mega \
                    // Пины ЭНКОДЕРА
...
//****************************************************************************************************************
//********** ИНИЦИАЛИЗАЦИЯ **********
LEDdata Leds[NUM_SHOTS];  // буфер ленты типа LEDdata (размер зависит от
                          // COLOR_DEBTH)
microLED Strip1(Leds, NUM_SHOTS, LED1_PIN);  // объект лента
#ifdef LED_TOWER                             //
#define NUMLEDS 16  // колличество светиков во второй ленте
LEDdata
    Leds2[NUMLEDS];  // буфер ленты типа LEDdata (размер зависит от COLOR_DEBTH)
microLED Strip2(Leds2, NUMLEDS, LED2_PIN);  // объект лента
encMinim Enc(Lcd_CLK, Lcd_DT, Lcd_SW,
             0);  // пин Lcd_CLK, пин Lcd_DT, пин Lcd_SW, направление (0/1)
LCD_1602_RUS lcd(0x27, 16, 2);  //Адрес дисплея 0x27 или 0x3F, подключение
                                //А4-SDA-зеленый, А5-SCL-желтый
...
uint8_t
    shotPos[NUM_SHOTS];  // = 0; // позиция рюмок, в сетапе считается из памяти.
...
uint8_t TostList[100];  // массив номеров треков тостов, в нём перемешаем всё,
                        // максимум 100 тостов. нужно ли столько?
uint8_t MusicList[100];  // массив номеров песенок, в нём перемешаем всё,
                         // максимум 100
uint8_t ManRum = 1;
uint8_t mixTost = 0;
uint8_t mixMusic = 0;
...
uint8_t battery0[8] = {B01110, B11111, B10001, B10001,
                       B10001, B10001, B10001, B11111};  // пустая батарея
uint8_t battery1[8] = {B01110, B11111, B10001, B10001,
                       B10001, B10001, B11111, B11111};  // батарея одно деление
uint8_t battery2[8] = {B01110, B11111, B10001, B10001,
                       B10001, B11111, B11111, B11111};  // батарея два деления
uint8_t battery3[8] = {B01110, B11111, B10001, B10001,
                       B11111, B11111, B11111, B11111};  // батарея три деления
uint8_t battery4[8] = {B01110, B11111, B10001, B11111, B11111,
                       B11111, B11111, B11111};  // батарея четыре деления
uint8_t battery5[8] = {B01110, B11111, B11111, B11111,
                       B11111, B11111, B11111, B11111};  // батарея полная
uint8_t power[8] = {B01010, B01010, B11111, B11111,
                    B11111, B01110, B00100, B00100};  // питание от сети
#endif
...
void setup() {
    ...
    delay(100);
    myMP3.begin(
        Serial3);  //инициализируем Serial порт МП3 плейера 14 и 15 Пины меги
#endif

    //********** настройка пинов **********
    pinMode(PUMP_POWER, OUTPUT);  // Помпа
#if (PUMP_LEVEL == 0)
    digitalWrite(PUMP_POWER,
                 HIGH);  // высокий уровень если активный на включение ноль
#else
    digitalWrite(PUMP_POWER,
                 LOW);  // низкий уровень если активна на включение единица
#endif
...
#if (SWITCH_LEVEL == 0)
        pinMode(SW_pins[i],
                INPUT_PULLUP);  // вход с подтяжкой если активный ноль
#else
    ...
    // Procent = 5; // используем переменную счётчика, потом приведём её в
    // нужное значение, она учавствует в другом месте.
    for (uint8_t i = 0; i < NUM_SHOTS; i++) {
        EEPROM.get(address,
                   shotPos[i]);  // считываем из памяти положение для сервы
        if (shotPos[i] < 1 || shotPos[i] > 180)
            shotPos[i] = Procent;  // если ячейки памяти не в интервале, то
    ...
    for (uint8_t i = 0; i < NUM_SHOTS; i++) {
        EEPROM.get(
            address,
            ManDrink[i]);  // считываем из памяти, дриньки для мультиразлива
        if (ManDrink[i] < 20 || ManDrink[i] > MAX_DRINK)
            ManDrink[i] = 20;  // если ячейки памяти не в интервале, то ставим
                               // начальные значения
        address++;
    }
    ...
    address = 70;
    EEPROM.get(address,
               mixTost);  // считываем из памяти флаг перемешивания тостов
    if (mixTost > 1) mixTost = 0;

    address = 80;
    EEPROM.get(address,
               folder);  // считываем из памяти номер папки треков тостов
    if (folder < 1 || folder > 9) folder = 1;
    oldFolder = folder;

    address = 90;
    EEPROM.get(address,
               noDoliv);  // считываем из памяти флаг перемешивания тостов
    ...
    address = 110;
    EEPROM.get(address,
               mixMusic);  // считываем из памяти флаг перемешивания песен
    ...
    delay(100);  //Между двумя командами необходимо делать задержку 100
                 //миллисекунд, в противном случае некоторые команды могут
                 //работать не стабильно.
    myMP3.EQSelect(EQ_NORMAL);
    delay(100);
    tracks = myMP3.numTracksInFolder(
        folder);  // считываем колличество треков в папке 01-09 в корне флешки,
                  // не больше 100
    ...
    if (FLOWdebounce.isReady()) {
        for (uint8_t i = 0; i < NUM_SHOTS; i++) {
            ...
            if (shotStates[i] == NO_GLASS && swState &&
                readySystem) {  // поставили пустую рюмку
                shotStates[i] = EMPTY;  // флаг на заправку
#ifndef SERVO_CHANGE_DIRECTION
                Strip1.setLED(i, mRGB(255, 0, 0));  // подсветили красный
#else
                Strip1.setLED(NUM_SHOTS - 1 - i,
                              mRGB(255, 0, 0));  // подсветили красный
#endif
                LEDchanged = true;
                // DEBUG("set glass");
                // DEBUG(i);
                ...
                if (barMan == 1 && (MenuFlag == 1 || MenuFlag == 2) &&
                    !systemON) {
                    oled_Nalivau();
                    systemON = true;
                    if (ledShowOn) {
            ...
            if (shotStates[i] != NO_GLASS &&
                !swState) {  // убрали пустую/полную рюмку
                ...
                if (i == curPumping) {  // убрали во время заправки!
                    ...
                    if (MenuFlag == 20 || promivka)
                        ...
                        Play_track(2);  // трек в папке mp3,  кто то снял рюмку
                                        // при наливе
                }
                // DEBUG("take glass");
                // DEBUG(i);
    ...
    switch (systemState) {
...
#if (SOUND_THEME == 1)
                            if (digitalRead(BUSY_PIN) && !promivka &&
                                MenuFlag != 20) {
                                Play_track(53);  //  трек в папке mp3 , движения
                                                 //  башни танка
                            }
#else
                            if ((DrinkCount == 0 || digitalRead(BUSY_PIN)) &&
                                !promivka && MenuFlag != 20) {
                                Play_track(18);  // трек в папке mp3 , мелодия
                                                 // во время разлива
                            }
#endif
                            curPumping = i;  // запоминаем выбор
                            systemState = MOVING;  // режим - движение
                            shotStates[curPumping] =
                                IN_PROCESS;  // стакан в режиме заполнения
                            break;
                        }
                    }
                }
            ...
            if (noGlass) {  // если не нашли ни одной рюмки
                ...
                if (!moving) {  // едем до упора
                    ...
                    if (DrinkCount > 0 && !promivka && MenuFlag != 20) {
                        ...
                        returnMenu = true;
                        Play_track(3);  // трек в папке mp3,  типа
                                        // предупреждение нет рюмок
        ...
        case MOVING:  // движение к рюмке
            ...
            if (!moving) {              // если приехали
                ...
                if (!promivka)
                    TIMEProcent.setInterval(time50ml /
                                            50);  // перенастроили таймер,
                else
        ...
        case PUMPING:  // если качаем
            if (TIMEProcent.isReady()) {
                Procent++;
                if (!promivka) {
                    if ((MenuFlag == 1 || MenuFlag == 20 || MenuFlag == 6) &&
                        Procent >= Drink)
                        readyDrink = true;
...
#else
                Strip1.setLED(NUM_SHOTS - 1 - curPumping,
                              mRGB(0, 255, 0));  // подсветили
#endif
                Strip1.show();
                curPumping = -1;     // снимаем выбор рюмки
                systemState = WAIT;  // режим работы - ждать
...
#ifdef START_POS_SERVO_GLASS1
                            Servo_move(
                                shotPos[0]);  // цель серво - первая рюмка
#else
                            Servo_move(0);       // цель серво - 0
#endif
                        } while (moving);
...
void Servo_move(uint8_t target) {
    ...
    if (deadTime &&
        millis() - prevServoTime >=
            POST_PAUSE) {  // пауза после остановки сервы до включения помпы
        deadTime = false;
        moving = false;
    }
}
...
void Mon_Battery() {
    ...
    if (!systemON && !ledShow && !playMush &&
        millis() - prevBatTime >= 1000UL) {
        prevBatTime = millis();
        ...
        if (!lowBat && flagBat != flag && MenuFlag <= 6 && !tost &&
            !returnMenu) {
            flag = flagBat;
            switch (flag) {
                case 0:
        ...
        if (MenuFlag == 18) {
            ...
            // lcd.setCursor(12, 1);     // раскомментировать, нужно для отладки
            // заряда АКБ lcd.print(value, DEC);    // раскомментировать, нужно
            // для отладки заряда АКБ
        }
    }
}
#endif
...
void mix() {
    if (tracks != -1) {
        for (uint8_t i = 0; i < tracks; i++)
            TostList[i] =
                i;  // заполняем массив тостов, последовательно значениями
        num = 0;
        if (mixTost == 1) {
            randomSeed(CreateTrulyRandomSeed());  // инициализирует генератор
                                                  // псевдослучайных чисел
            for (uint8_t i = tracks - 1; i > 0;
                 i--) {  // перемешиваем значения случайно в массиве так, что
                         // они не повторяются
...
void mix_music() {
    if (tracks2 != -1) {
        for (uint8_t i = 0; i < tracks2; i++)
            MusicList[i] =
                i;  // заполняем массив песенок, последовательно значениями
        if (mixMusic == 1) {
            randomSeed(CreateTrulyRandomSeed());  // инициализирует генератор
                                                  // псевдослучайных чисел
            for (uint8_t i = tracks2 - 1; i > 0;
                 i--) {  // перемешиваем значения случайно в массиве так, что
                         // они не повторяются
                uint8_t idx = random(i + 1);
...
void play_next() {
    ...
    if (nextTrack) {
        ...
        if (num2 <= tracks2 - 1) {
            ...
            if (!player) {
                player = true;
                if (MenuFlag == 3) oled_Player(0);
            } else {
                if (MenuFlag == 3) oled_Player(4);
            }
        } else {
            player = false;
            num2 = 0;
...
void Button_Tower() {
    ...
    if ((MenuFlag == 1 || MenuFlag == 2 || MenuFlag == 6) &&
        ((!systemON && !returnMenu && !save && !playMush && barMan == 0) ||
         promivka)) {
        ...
        if (flagDubl &&
            millis() - butTowTime >= 400) {  // двойной клик на башне
            flagDubl = false;
            if (clickCount >= 2) {
                if (MenuFlag != 6) {
                    systemON = true;
...
void play_Mushketery() {
    ...
    if (playMush) {
        ...
        if (digitalRead(BUSY_PIN) || player) {
            ...
            systemON = true;
            flag =
                true;  // флаг ,если пустые рюмки найдены показываем меню налива
            readySystem = false;
            countRand = 0;
            ...
            for (int8_t i = NUM_SHOTS - 1; i >= 0; i--) {
                ...
                rumka[idx] = t;
                if (shotStates[i] == EMPTY)
                    countRand++;  // снова подсчёт стопок готовых к наливу,
                                  // вдруг кто снял во время трека рулетки
            }
            ...
            for (uint8_t i = 0; i < NUM_SHOTS; i++) {
                ...
                if (countRand > 1 && subMush == 2) {
                    if (i == randomGlass && rumka[i] == -1)
                        ...
                        rumka[i] = -1;  // убираем из разлива одну из
                                        // установленных рюмок
                }
            }
            playMush = false;
...
//***********************************************************************************************************
void move_enc(uint8_t *var, int16_t shift, int16_t lowLimit, int16_t upLimit,
              bool cycle) {
...
void move_enc(int16_t *var, int16_t shift, int16_t lowLimit, int16_t upLimit,
              bool cycle) {
    *var += shift;
...
void move_enc(int8_t *var, int16_t shift, int16_t lowLimit, int16_t upLimit,
              bool cycle) {
    *var += shift;
    if (*var > upLimit) *var = (cycle) ? lowLimit : upLimit;
    if (*var < lowLimit) *var = (cycle) ? upLimit : lowLimit;
...
void EncTick() {
    ...
    if (!save && !systemON && !tost && !playMush) {
        if (Enc.isTurn()) {  // если произошло движение энкодера в любую сторону
            ...
            switch (MenuFlag) {
                case 0:  // главное меню
                    move_enc(&Menu, drift, 0, 4,
                             true);  // Перемещение  по главному меню
                    oled_main_screen();
                    break;
...
#ifdef BAT_MONITOR_ON
                    move_enc(&subNastr, drift, 1, 9,
                             true);  // Перемещение  по меню настроек
#else
                    move_enc(&subNastr, drift, 1, 8,
                             true);  // Перемещение  по меню настроек
#endif
                    oled_Nastroiki();
                    break;
                ...
                case 5:  // меню выбора игры мушкетёров
                    move_enc(&subMush, drift, 1, 2,
                             true);  // Перемещение  по мушкетёрам
                    oled_Mushketery();
                    break;

                case 12:  // меню настройки дриньков мультиразлива
                    move_enc(&ManDrink[(ManRum - 1)], drift * 5, 20, MAX_DRINK,
                             false);
                ...
                case 30:  // меню выбора настройки тостов
                    move_enc(&subMenu, drift, 1, 3,
                             true);  // Перемещение  по меню тостов
                    oled_Tost();
                    break;
                ...
                case 33:  // меню выбора папки тостов
                    move_enc(&folder, drift, 1, 9,
                             true);  // Перемещение  по меню папок
                ...
                case 40:  // меню выбора настроек серво
                    move_enc(&subMenu, drift, 1, 2,
                             true);  // Перемещение  по меню настроек серво
                    oled_servo();
                    break;
        ...
        if (Enc.isHolded()) {  // длительное нажатие кнопки энкодера
            if (MenuFlag == 11 && !promivka) {
                ...
                oled_main_screen();
                if (MenuFlag == 3) {  // если были в меню плеера , то обновляем
                                      // громкость плеера и рандом
#ifdef MEMORY_ON
                    address = 100;
                ...
                MenuFlag = 0;
            } else if (MenuFlag == 12) {  //Выход из меню настроек мультиразлива
                                          //в главное меню настроек
                MenuFlag = 4;
                oled_Nastroiki();
#ifdef MEMORY_ON
                address = 10;
                for (uint8_t i = 0; i < NUM_SHOTS; i++) {
                    EEPROM.update(address,
                                  ManDrink[i]);  // обновляем в памяти, дриньки
                                                 // для мультиразлива
                    address++;
                    delay(5);
                }
                ...
                EEPROM.update(address, volume);
                       ...
                       32) {  //Выход из меню перемешивания  в меню тоста
                MenuFlag = 30;
                oled_Tost();
#ifdef MEMORY_ON
                address = 70;
                EEPROM.update(
                    address,
                    mixTost);  // обновляем в памяти флаг перемешивания тостов
#endif
                ...
                address = 80;
                EEPROM.update(address,
                              folder);  // обновляем в памяти выбор папок
#endif
            } else if (MenuFlag == 41) {  //Выход из меню  калибровки  сервы в
                                          //меню настроек сервы
                MenuFlag = 40;
                count = -1;
                servoPos = 0;
                ...
                for (uint8_t i = 0; i < NUM_SHOTS; i++) {
                    EEPROM.update(
                        address,
                        shotPos[i]);  // обновляем в памяти положение для сервы
                    address++;
                    delay(5);
                }
#endif
                ...
                address = 120;
                EEPROM.update(address,
                              barMan);  // обновляем в памяти режим бармена
#endif
            } else if (MenuFlag == 42) {  // выход из настроки скорости серво в
                ...
                address = 40;
                EEPROM.update(address,
                              speedSer);  // обновляем в памяти скорость сервы
#endif
#ifdef BAT_MONITOR_ON
            } else if (MenuFlag == 18) {  // выход из меню просмотра напряжения
                                          // батареи в меню настроек
                MenuFlag = 4;
                oled_Nastroiki();
#endif
            } else if (MenuFlag == 19) {  // выход из меню настройки таймера сна
                                          // в менюнастроек
                MenuFlag = 4;
                oled_Nastroiki();
#ifdef MEMORY_ON
                address = 50;
                EEPROM.update(address,
                              sleepTime);  // обновляем в памяти таймер сна
#endif
                if (sleepTime != 0) SAVEtimer.setInterval(sleepTime * 30000UL);
            } else if (MenuFlag ==
                       20) {  // выход из меню настройки помпы в меню настроек
                MenuFlag = 4;
                do {
#ifdef START_POS_SERVO_GLASS1
                    Servo_move(shotPos[0]);  // цель серво - первая рюмка
                ...
                address = 60;
                EEPROM.put(address,
                           time50ml);  // обновляем в памяти время налива 50 мл.
#endif
            } else if (MenuFlag == 30 ||
                       MenuFlag == 40) {  // выход из меню настройки тостов и
                                          // серво в меню настроек
                MenuFlag = 4;
                oled_Nastroiki();
                if (playOn) player = true;
            } else if (MenuFlag == 6) {  // выход из меню розлива мушкетёров  в
                                         // меню мушкетёров
                MenuFlag = 5;
                oled_Mushketery();
            }
    ...
    if (Enc.isSingle()) {  // одиночный клик
        ...
        if (save) {
            ...
            if (systemON || playMush) {  // нажата кнопка во время работы
                                         // наливатора, то отбой всего
                systemState = SEARCH;
                DrinkCount = 0;
                ...
                returnMenu = true;
            } else if (tost) {  // если во время произношения тоста нажата
                                // кнопка, то отменяем воспроизведение
                if (!digitalRead(BUSY_PIN)) {
                    noTost = true;
                    if (ledShowOn) {
                        ledShow = false;
                ...
                oled_Auto();
                       ...
                       MenuFlag == 2) {  //Начинается автоматический разлив
                if (barMan == 0 && readySystem) {
                    systemON = true;
                    flag = true;  // флаг ,если пустые рюмки найдены показываем
                                  // меню налива
                    if (noDoliv == 1) readySystem = false;
#ifdef Button_Tower
                ...
                if (readySystem) flagMush = true;
                       ...
                       subMenu ==
                           1) {  // вошли из меню тост в настройку громкости
                MenuFlag = 31;
                oled_Volume(0);
            } else if (MenuFlag == 30 &&
                       subMenu == 2) {  // вошли из меню тост в настройку
                                        // перемешивания треков
                MenuFlag = 32;
                oled_mix_Tost();
            } else if (MenuFlag == 30 &&
                       subMenu == 3) {  // вошли из меню тост в настройку папки
                                        // проигрывания
                MenuFlag = 33;
                oled_Num_Folder(0);
            } else if (MenuFlag == 40 &&
                       subMenu == 1) {  // вошли из меню сервы в настройку
                                        // положения сервы над рюмками
                MenuFlag = 41;
                oled_Servo_calibr(0);
            } else if (MenuFlag == 40 &&
                       subMenu == 2) {  // вошли из меню сервы в настройку
                                        // скорости сервы
                MenuFlag = 42;
                oled_Servo_speed(0);
            ...
            } else if (MenuFlag == 42) {  // двигаем серво туда-обратно для
                                          // наглядности установленной скорости
                do {
                    Servo_move(180);
                } while (moving);
                ...
                do {
                    ...
                    Servo_move(0);  // цель серво - 0
                       ...
                       20) {  // наливаем в рюмку для проверки величины налива,
                              // после изменения времени налива
                Drink = 50;
                systemON = true;
                oled_Kalibr_Pump(1);
            }
...
void CvetoMuzik() {
...
#endif
    if ((MenuFlag == 15 || ledShow || systemState == PUMPING) &&
        LEDtimer.isReady()) {
        if (systemState == PUMPING) {
            ...
            if (++led >= NUM_SHOTS) {
                ...
                if (col > 1350) {
                    col = 0;
                    if (++count ==
                        6) {  // колличество проходов иллюминации после налива
                        count = 0;
                        if (MenuFlag != 15 && !playMush)
                            check = true;  // если не в меню настроек, то
                                           // проверяем рюмки
                    }
                }
            }
    ...
    if (check) {  // проверка рюмок
        for (led = 0; led < NUM_SHOTS; led++) {
            ...
            if (shotStates[led] == READY)
                Strip1.setLED(led,
                              mRGB(0, 255, 0));  // налитая рюмка, статус: готов
    ...
    if ((MenuFlag == 15 || rainbow) && !clearLed && TOWERtimer.isReady()) {
        Leds2[count2] =
            mHSV((uint8_t)(count3 + count2 * (255 / NUMLEDS)), 255, 255);  //
        count3 += 4;
        Strip2.show();
        if (++count2 >= NUMLEDS) {
            count2 = 0;
            if (!playMush && MenuFlag != 15 &&
                ++count4 > 25) {  // продолжительность радуги у.е.
                rainbow = false;
                clearLed = true;
                TOWERtimer.reset();
            }
    ...
    if (systemON && ledShowOn && !clearLed && !promivka && MenuFlag != 20 &&
        TOWERtimer.isReady()) {
...
void Tost() {
    ...
    if (tost) {
        ...
        if ((readyTost &&
             ((PAUSEtimer.isReady() && digitalRead(BUSY_PIN)) || noTost)) ||
            volume == 0 || tracks == -1) {
            tost = false;
            if (noTost) {
                noTost = false;
