...
void Tost();                              // Экран и вывода тостов
...
#define Lcd_CLK 7        // Пины ЭНКОДЕРА для Arduino mega
...
void loop()
{
  ...
  {
#endif
    EncTick();         // Кнопки-крутилки
    flowTick();        // Наливайка, опрос концевиков
    CvetoMuzik();      // Светомузыка
    play_Mushketery(); // Режим МУШКЕТЕРЫ
    Energy_Saving();   // Энергосбережение
    Tost();            // Экран и вывода тостов
    ret_menu();        // Выход из некоторых меню
    play_next();       // Воспроизведение следующего трека
#ifdef Button_Tower
    Button_Tower(); // Обработка кнопки на башне
#endif
#ifdef BAT_MONITOR_ON
  }
  Mon_Battery(); // Монитор питания и защита батареи
#endif
}
...
void EncTick()
{
  ...
  if (!save && !systemON && !tost && !playMush)
  {
    ...
    { // длительное нажатие кнопки энкодера
      ...
      { //Выход в главное меню
        oled_main_screen();
        if (MenuFlag == 3)
        { // если были в меню плеера , то обновляем
      ...
      { //Выход из меню настроек мультиразлива
      ...
      { // выход из меню просмотра напряжения
  ...
  { // одиночный клик
    ...
    else
    {
      ...
      { // если во время произношения тоста нажата
      ...
      { // вошли из меню тост в настройку
        // перемешивания треков
        MenuFlag = 32;
        oled_mix_Tost();
      }
      else if (MenuFlag == 30 && subMenu == 3)
      { // вошли из меню тост в настройку папки
        // проигрывания
        MenuFlag = 33;
        oled_Num_Folder(0);
      }
      else if (MenuFlag == 40 && subMenu == 1)
      { // вошли из меню сервы в настройку
